...
export function splitStableBlocks(markdown: string): string[] {
    ...
    for (const token of tokens) {
        ...
        if (trimmedBlock === '$$' && mergedBlocksLen > 0) {
            ...
            const prevStartsWithDollar = startsWithDoubleDollar(previousBlock)
            const prevDollarCount = countDoubleDollars(previousBlock)

            if (prevStartsWithDollar && prevDollarCount % 2 === 1) {
