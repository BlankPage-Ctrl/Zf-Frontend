...
export function createLinkModel(options: LinkModelOptions): LinkModel {
    ...
    const loading =
        !!options.node.loading || !!options.hasLoadingNode?.(options.node.children) || !url
...
export async function checkTrustedLink(url: string, linkOptions?: LinkOptions): Promise<boolean> {
    if (linkOptions?.safetyCheck === false) return true

    const isTrusted = linkOptions?.isTrusted
    if (typeof isTrusted !== 'function') return false
