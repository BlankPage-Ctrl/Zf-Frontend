import type { MarkdownAstParserOptions, MarkdownAstParserResult, ParsedNode } from '@markmend/ast'
import type { AnimationSplit, AnimationType, BuiltinNodeRenderers, PreloadConfig } from '../types'
import type { StreamMarkdownMode, StreamMarkdownProcessed } from './types'
...
export function createProcessedMarkdownModel(
    processed?: MarkdownAstParserResult,
...
export function resolveEnableAnimate(mode: StreamMarkdownMode, enableAnimate?: boolean): boolean {
    if (typeof enableAnimate === 'boolean') return enableAnimate
...
export function resolveCaret(caret?: string): string | undefined {
    if (!caret || !(caret in CARETS)) return undefined
...
export function createRootStyle(
    ...
    if (duration !== undefined) style[STREAM_MARKDOWN_CSS_VARIABLES.animationDuration] = duration
    return style
}
...
export function resolvePreloadNodeRenderers<TBuiltinNodeRenderer extends string>(
    ...
    const resolvedFallback =
        fallback ?? (PRELOAD_NODE_RENDERER as unknown as readonly TBuiltinNodeRenderer[])
