import type { ParsedNode, TableCellNode, TableNode, TableRowNode } from '@markmend/ast'
import type {
    ControlDescriptor,
    DownloadEvent,
    MaybePromise,
...
export async function handleTableControlAction(
    ...
    if (options.key === 'download') {
        ...
        if (result) await options.saveFile?.(`table.${data.extension}`, data.content, data.mimeType)
    }
