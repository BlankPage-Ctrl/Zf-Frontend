...
export function getTableCellNodes<TNode = unknown>(cell: TNode | { children?: TNode[] }): TNode[] {
...
export function extractTableDataFromElement(tableElement: HTMLElement): TableData {
    ...
    for (const cell of headerCells) headers.push(cell.textContent?.trim() || '')
...
export function tableDataToCSV(data: TableData): string {
    ...
    const escapeCSV = (value: string): string => {
        ...
        if (hasQuote) return `"${value.replace(CSV_QUOTE_PATTERN, '""')}"`
