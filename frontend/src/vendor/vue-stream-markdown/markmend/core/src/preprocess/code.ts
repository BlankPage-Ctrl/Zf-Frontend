...
function removeTrailingIncompleteBackticks(content: string): string {
    ...
    if (backtickSequence.length === 1) {
        ...
        if (count % 2 === 1 && !isInCodeBlock) return content // Keep it, it's closing inline code
    ...
    if (backtickSequence.length === 3) {
        ...
        if (isInCodeBlock) return content // Keep it, it's closing a code block
