...
function isUnclosedHtmlFragment(fragment: string): boolean {
    ...
    return (
        htmlCommentStartPattern.test(fragment) ||
...
export function fixHtml(content: string): string {
    if (!content || isInsideUnclosedCodeBlock(content)) return content
    ...
    if (visibleEnd <= 0) return content

    const visibleContent = content.slice(0, visibleEnd)
    const fragmentStart = visibleContent.lastIndexOf('<')
    if (fragmentStart === -1) return content

    if (fragmentStart > 0 && visibleContent[fragmentStart - 1] === '\\') return content

    const fragment = visibleContent.slice(fragmentStart)
    if (!isUnclosedHtmlFragment(fragment)) return content

    const codeBlockRanges = findClosedCodeBlockRanges(content)
    if (isPositionInRanges(fragmentStart, codeBlockRanges)) return content

    const inlineCodeRanges = findInlineCodeRanges(content, codeBlockRanges)
    if (isPositionInRanges(fragmentStart, inlineCodeRanges)) return content

    const beforeFragment = content
        .slice(0, fragmentStart)
