...
import type { CodeNodeRendererProps, Control, SelectOption } from '../../types/index.ts'
import {
...
import { useContext, useControls, useI18n, useMermaid } from '../../composables/index.ts'
import { CODE_PREVIEWERS } from '../previewers/index.ts'
import Actions from './actions.vue'
import { LANGUAGE_ICONS } from './language-icons.ts'
...
const codeBlockModel = computed(() =>
    createCodeBlockModel<Component>({
        node: props.node,
...
const builtinControls = computed((): Control[] =>
    createCodeBlockControlDescriptors({
        ...
        onClick: (_event: MouseEvent, select?: SelectOption) =>
            handleControlClick(item.key, select),
...
const modalControls = computed(() =>
    resolveControls<CodeNodeRendererProps>('code', headerControls.value, props).filter(
        (i) => i.key !== 'collapse',
    ),
)
...
<template>
    ...
    <div
        ...
        :class="[{ 'code-loading': props.node.loading }]"
    >
        <header data-stream-markdown="code-block-header" class="code-block-header">
            <slot name="title">
            ...
            <slot name="actions">
                <div data-stream-markdown="actions" class="flex gap-1 items-center">
                    <PreviewSegmented
                    ...
                    <button class="details-toggle" @click="collapsed = !collapsed" type="button">
        ...
        >
            ...
            />
            <CodeNode v-show="mode === 'source'" v-bind="props" :show-header="false" />
        </component>
    </div>
</template>
