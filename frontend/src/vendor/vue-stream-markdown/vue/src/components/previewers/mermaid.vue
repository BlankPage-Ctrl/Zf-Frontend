...
function updateHeight() {
    ...
    if (height) previewState.value = setMermaidMeasuredHeight(previewState.value, height)
}
...
const mermaidControls = computed((): Control[] =>
    resolveControls<CodeNodeRendererProps>('mermaid', [], props),
)

watch(() => [code.value, mermaidOptions.value, isDark.value, nodeLoading.value], render, {
    immediate: true,
})
...
watch(
    loading,
    (curr, prev) => {
        if (shouldEagerRenderMermaidAfterLoading(previewState.value, curr, prev)) {
            eagerRender()
        }
    },
    { immediate: true },
