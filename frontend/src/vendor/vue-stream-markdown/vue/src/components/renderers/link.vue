...
import type { LinkNodeRendererProps } from '../../types/index.ts'
import { checkTrustedLink, createLinkModel, openExternalUrl } from '@stream-markdown/core'
import { useClipboard } from '@vueuse/core'
import { computed, ref } from 'vue'
import { useContext, useI18n, useSanitizers } from '../../composables/index.ts'
...
async function handleClick(event: MouseEvent) {
    if (!transformedUrl.value) return
...
function handleConfirm() {
    if (transformedUrl.value) openExternalUrl(transformedUrl.value)
    handleClose()
}

async function handleCopy() {
    if (transformedUrl.value) copy(transformedUrl.value)
}
