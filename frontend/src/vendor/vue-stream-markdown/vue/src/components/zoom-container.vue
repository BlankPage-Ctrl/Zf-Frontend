...
const controls = computed((): Control[] =>
    model.value.controls.map((item) => {
        const { visible: _visible, ...rest } = item
        return {
            ...rest,
            name: item.label ?? t(item.labelKey ?? ''),
            onClick: (_event: MouseEvent, select?: SelectOption) => {
                if (item.key === 'zoomIn') zoomIn()
                else if (item.key === 'zoomOut') zoomOut()
                else if (item.key === 'updateZoom') updateZoom(select)
            },
        }
    }),
)
...
<template>
    ...
    >
        ...
        <div data-stream-markdown="zoom-inner" class="flex size-full items-center justify-center">
            <div
