...
export function useFloating(options: UseFloatingOptions) {
    ...
    const getAppendTo = () => {
        if (!isClient()) return options.getContainer?.() || 'body'
        const target = options.getContainer?.() || parentEl.value
        if (target instanceof HTMLElement && !isElementInDocumentBody(target)) return 'body'
        return target || 'body'
    }
    ...
    function handleClickOutside(event: MouseEvent) {
        ...
        if (!isClickInsideFloating(target, referenceEl.value, floatingEl.value)) open.value = false
    }
