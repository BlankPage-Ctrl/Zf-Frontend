...
type RequiredSchema = Required<Omit<ButtonSchema, 'label' | 'icon' | 'href' | 'to' | 'preset' | 'overrides' | 'ariaLabel' | 'ariaExpanded' | 'ariaPressed' | 'ariaDescribedby'>>
...
export function resolveButtonSchema(schema: ButtonSchema): ResolvedButton {
  ...
  const isIconOnly  = !!(merged.iconPosition === 'only' || (merged.iconPosition == null && merged.icon && !merged.label))
