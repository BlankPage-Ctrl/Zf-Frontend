  ...
  if (newVal) {
    ...
    setTimeout(() => {
      ...
      if (focusables.length > 0) {
        ...
        const firstInput = dialogRef.value?.querySelector('input, select, textarea') as HTMLElement;
        if (firstInput) {
          firstInput.focus();
        } else {
          focusables[0]?.focus();
...
.width-sm { max-width: 400px; }
.width-md { max-width: 560px; }
.width-lg { max-width: 720px; }
.width-xl { max-width: 960px; }
