...
import type { DropdownItemConfig, DropdownMode, DropdownPlacement, WidthStrategy, TriggerMode, CommandAction } from './types'
...
const floatingMiddleware = computed(() => {
  const mw = [
    ...
    flip({
      fallbackPlacements: props.placement === 'top'
...
function handleItemClick(item: DropdownItemConfig) {
  ...
  if (item.value !== undefined) {
    ...
    if (props.mode === 'select') {
      ...
      if (idx >= 0) {
        emit('update:modelValue', current.filter((v) => v !== item.value))
      } else {
  ...
  >
    ...
    >
      <slot name="trigger" :is-open="isOpen" :toggle="toggle">
        <span class="dropdown-trigger__default" @click="toggle">{{ isOpen ? 'Close' : 'Open' }} Dropdown</span>
      </slot>
    </div>
