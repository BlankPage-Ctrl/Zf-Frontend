...
import type { ListDirData, ReadFileData, GetStatData, WatchEvent } from '../../../src/fm/types/index.js';
...
export const filesApi = {
  listDir: (workspaceId: string, path: string) =>
    request<ListDirData>(`/workspaces/${workspaceId}/files?path=${encodeURIComponent(path)}`).then(toFEListDirData),

  getStat: (workspaceId: string, path: string) =>
    request<GetStatData>(`/workspaces/${workspaceId}/files/stat?path=${encodeURIComponent(path)}`),
