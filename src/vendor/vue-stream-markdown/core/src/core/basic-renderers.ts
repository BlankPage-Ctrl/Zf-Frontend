...
export function createParagraphModel(options: ParagraphModelOptions) {
    ...
    const marginBottom = parentNodeType !== 'listItem' && nextNodeType === 'list' ? '0.5rem' : ''
    const nodeTypes = collectNodeTypes(options.node.children)
    const lineHeight = options.deep === 0 && nodeTypes.every(isPlainTextNodeType) ? 1.75 : ''
...
export function createListModel(node: ListNode) {
    const isTaskList = node.children.some(
        (child) => typeof (child as ListItemNode).checked === 'boolean',
    )
    const tag = node.ordered ? 'ol' : 'ul'
    const id = isTaskList ? 'task-list' : node.ordered ? 'ordered-list' : 'unordered-list'
