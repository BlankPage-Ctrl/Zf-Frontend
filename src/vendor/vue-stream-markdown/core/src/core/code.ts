...
import { LANGUAGE_ALIAS, LANGUAGE_EXTENSIONS } from '../constants'
import { getConfigValue, isCodeOptionEnabled, normalizeCssSize, resolveCodeOptions } from '../utils'
...
export function createCodeBlockModel<TComponent = unknown>(
    ...
    return {
        ...
        language,
        progressiveRender,
        previewPlacement: resolvePreviewPlacement(
            options.previewers,
            codeOptionsModel.showLanguageTitle,
        ),
        previewable,
        inlineInteractive:
            getConfigValue<boolean>(options.controls, 'mermaid.inlineInteractive') ?? true,
...
export function shouldProgressivelyRenderPreview<TComponent = unknown>(
    ...
    if (typeof previewers === 'boolean') return language === 'mermaid'
...
export function isCodePreviewable<TComponent = unknown>(
    ...
    if (previewers === false) return false

    const progressiveRender =
        options.progressiveRender ?? shouldProgressivelyRenderPreview(previewers, options.language)

    if (!progressiveRender && options.nodeLoading) return false
    ...
    if (previewers === true || previewers === undefined) {
        if (options.language === 'html' && html) return true
        if (options.language === 'mermaid' && mermaid) return true
        return false
    }

    if (typeof previewers === 'object') {
        if (previewers.components?.[options.language] === false) return false
        ...
        if (
            isCustomPreviewComponent(component, options.isPreviewComponent) &&
            (progressiveRender || !options.nodeLoading)
        )
            return true
    }
...
export function resolveCodePreviewComponent<TComponent = unknown>(
    ...
    if (!previewers || typeof previewers === 'boolean') return previewer

    const data = previewers.components?.[language]
    if (data === false) return previewer

    if (isCustomPreviewComponent(data, isPreviewComponent)) return data as TComponent
...
export function syncCodeBlockMode(
    state: CodeBlockModeState,
    previewable: boolean,
): CodeBlockModeState {
    if (!previewable || state.mode === 'preview') return state
    return { mode: 'preview' }
}
...
export async function handleCodeBlockControlAction(
    ...
    if (options.key === 'copy') {
        if (!options.node.value) return state
    ...
    if (options.key !== 'download' || options.node.loading) return state
...
async function downloadMermaid(options: CodeBlockControlActionOptions, format: 'svg' | 'png') {
    const result = await resolveBeforeDownload(options.beforeDownload, {
        type: 'mermaid',
        content: options.node.value,
    })
    if (result) await options.saveMermaid?.(format, options.node.value)
}
...
function isCustomPreviewComponent(
    ...
    if (isPreviewComponent) return isPreviewComponent(component)
