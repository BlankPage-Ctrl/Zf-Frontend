...
export function createImagePreviewSources(
    ...
    visitNodes(nodes, (node) => {
        ...
        if (!transformed || seen.has(url)) return
...
export function resolveImageControlPosition(controls: unknown): ZoomControlPosition {
    const position = getConfigValue<ZoomControlPosition | boolean>(
        controls,
...
export function createImagePreviewControlDescriptors(
    options: Pick<ImagePreviewModelOptions, 'controls' | 'hasDownload' | 'icons'> & {
        imageCount?: number
        imageSrc?: string
    },
): ImagePreviewControlDescriptor[] {
    const enableDownload =
        isConfigEnabled(options.controls, 'image.download') && !!options.hasDownload
    ...
    return [
        ...
        },
        {
            ...
            buttonStyle: options.icons?.arrowRight ? undefined : { transform: 'scaleX(-1)' },
            visible: imageCount > 1 && enableCarousel,
        },
        {
            key: 'flipX',
        ...
        },
        {
            ...
            buttonStyle: options.icons?.flipVertical ? undefined : { rotate: '90deg' },
            visible: enableFlip,
        },
        {
            key: 'rotateLeft',
        ...
        },
        {
            ...
            buttonStyle: options.icons?.rotateRight ? undefined : { transform: 'scaleX(-1)' },
            visible: enableRotate,
        },
    ]
}
...
function visitNodes(nodes: ParsedNode[], visit: (node: ParsedNode) => void) {
    nodes.forEach((node) => {
        ...
        if (Array.isArray(children)) visitNodes(children, visit)
    })
}
