...
export function createIconModel(options: { width?: number; height?: number }) {
...
function resolveErrorIcon(options: {
    ...
    if (options.hasIcon?.(options.variant)) return options.variant

    const name = options.variant.replace('harden-', '')
    if (options.hasIcon?.(name)) return name
