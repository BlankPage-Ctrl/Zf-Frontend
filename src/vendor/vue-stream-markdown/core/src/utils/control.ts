...
export function resolveVisibleControls<
    ...
    TProps,
>(config: unknown, type: string, builtinControls: TControl[], props: TProps): TControl[] {
    const customize = getConfigValue<ControlTransformer<TControl, TProps>>(
        config,
        `${type}.customize`,
    )
    if (typeof customize !== 'function') return filterVisibleItems(builtinControls)
