...
export function zoomInState(state: ZoomState, options: ZoomOptions = {}): ZoomState {
    ...
    if (state.zoom >= maxZoom) return state
...
export function zoomOutState(state: ZoomState, options: ZoomOptions = {}): ZoomState {
    ...
    if (state.zoom <= minZoom) return state
...
export function createDraggedState(
    point: ZoomPoint,
    dragStart: ZoomPoint,
...
export function createTouchZoomState({ state, rect, touches }: TouchZoomInput): TouchZoomState {
...
export function createTouchMovedState({
    ...
    const offsetY = centerY - rect.top - containerCenterY
    const contentFocalX =
        (touchState.centerX - touchState.initialTranslateX) / touchState.initialZoom
    const contentFocalY =
        (touchState.centerY - touchState.initialTranslateY) / touchState.initialZoom
...
export function resolveZoomControlPosition(
