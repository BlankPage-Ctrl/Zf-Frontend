...
function normalizeShikiModule(module: typeof import('shiki')): typeof import('shiki') {
    ...
    try {
        defaultExport = (module as typeof import('shiki') & { default?: typeof import('shiki') })
            .default
    } catch {
        defaultExport = undefined
    }

    if (defaultExport && typeof defaultExport === 'object') return { ...module, ...defaultExport }
...
export function createShikiRuntime(options: CodeRuntimeOptions = {}): ShikiRuntime {
    ...
    async function getThemes() {
        ...
        return [lightTheme, darkTheme].filter((theme) =>
            bundledThemesInfo.find((t) => t.id === theme),
        )
    }
    ...
    async function getDualTheme() {
        ...
        return {
            light:
                bundledThemesInfo.find((t) => t.id === lightTheme)?.id ?? DEFAULT_SHIKI_LIGHT_THEME,
    ...
    async function preload() {
        ...
        if (await hasShiki()) await getHighlighter()
    }
