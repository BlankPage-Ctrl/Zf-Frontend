...
export function resolveAllowedTags(options: HtmlPluginOptions = {}): string[] {
    ...
    return normalizeAllowedTags([...resolveTagList(allowedTags), ...resolveComponentTags(options)])
}
...
function resolveAllowedAttributes(
    ...
    allowedAttributes:
        | HtmlPluginOptions['allowedAttributes']
        | sanitizeHtmlLib.IOptions['allowedAttributes'],
): sanitizeHtmlLib.IOptions['allowedAttributes'] {
    if (allowedAttributes === false) return false
    ...
    if (isAllowedAttributeArray(allowedAttributes)) {
        return Object.fromEntries(allowedTags.map((tag) => [tag, [...allowedAttributes]]))
    }
...
function normalizeAllowedAttributes(
    ...
    return Object.fromEntries(
        Object.entries(allowedAttributes).map(([tag, attrs]) => [tag.toLowerCase(), [...attrs]]),
    )
}
