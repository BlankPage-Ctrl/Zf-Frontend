...
export class BeautifulMermaidRenderer extends MermaidRenderer {
    ...
    private get currentTheme() {
        const [light, dark] =
            resolveGetter(this.options.beautifulTheme) ?? DEFAULT_BEAUTIFUL_MERMAID_THEME
    ...
    private async getRenderOptions() {
        ...
        if (presetOptions) return { ...presetOptions, ...this.mergedOptions }
    ...
    async parse(code: string): Promise<MermaidParseResult> {
        ...
        if (!mightBeSupported(diagramType)) return this.getVanillaRenderer().parse(code)
    ...
    async render(code: string): Promise<MermaidRenderResult> {
        const diagramType = extractDiagramType(code)
        if (!mightBeSupported(diagramType)) return this.getVanillaRenderer().render(code)
    ...
    async isEnabled(): Promise<boolean> {
        ...
        try {
            if (await hasBundledBeautifulMermaidModule()) return true
