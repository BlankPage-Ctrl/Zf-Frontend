...
export class MarkdownAstParser {
    ...
    private syncAstsLoadingState() {
        ...
        for (const ast of this.asts) clearLoading(ast.children)
        ...
        if (lastLeafNode?.type === 'text') lastLeafNode.loading = true
    }
    ...
    private update(data: string) {
        ...
        const blocks =
            this.mode === 'static' && this.contents.length <= 1
                ? [data]
        ...
        const applyLoadingState = (
            ...
            if (!tailTextLoading) return updated
                    ...
                    this.mode === 'streaming'
                        ? this.processor.preprocess(content, {
                              singleDollarTextMath:
                                  this.options.mdastOptions?.singleDollarTextMath ?? false,
                          })
                        : content
            }
    ...
    private updateAstLoading(ast: SyntaxTree, loading: boolean) {
        ...
        if (!node) return ast
        if (Boolean(node.loading) === loading) return ast
        return this.updateNodeLoading(ast, node, loading)
    }

    private markLastTextNodeLoading(ast: SyntaxTree) {
        if (this.mode !== 'streaming') return ast

        const node = findLastLeafNode(ast.children)
        if (!node || node.type !== 'text') return ast
        if (node.loading) return ast
    ...
    private updateNodeLoading(
        ...
        const cloneNode = (node: ParsedNode): [ParsedNode, boolean] => {
            ...
            if (!children || !children.length) return [node, false]
    ...
    parseMarkdown(content: string): MarkdownAstParserResult {
        if (!content) return { contents: [], asts: [] }
        this.update(content)
        if (!this.asts.length) return { contents: [], asts: [] }
