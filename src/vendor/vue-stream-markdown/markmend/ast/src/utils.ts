...
export function resolveBuiltinExtensions<
    ...
    Ext = ReturnType<Builtins[Key]> extends (infer U)[] ? U : ReturnType<Builtins[Key]>,
>(
    builtins: Builtins,
    ctx: Ctx,
    ...
    for (const key in builtins) {
        ...
        const factory = typeof action === 'function' ? action : builtins[key]
        if (!factory || typeof factory === 'boolean') continue
    ...
    if (extend?.length) result.push(...extend)
    return result
}
...
export function findNodeParent(
    ...
    for (let i = nodes.length - 1; i >= 0; i--) {
        if (nodes[i] === targetNode && parent) return { parent, index: i }
