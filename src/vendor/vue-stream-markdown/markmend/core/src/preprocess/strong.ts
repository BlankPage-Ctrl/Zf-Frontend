...
export function fixStrong(
    ...
    const lastParagraphWithoutCodeBlocksAndUrlsAndMath = removeMathBlocksFromText(
        lastParagraphWithoutCodeBlocksAndUrls,
        options,
    )
    ...
    const asteriskMatches =
        lastParagraphWithoutCodeBlocksAndUrlsAndMath.match(doubleAsteriskPattern)
    ...
    const underscoreMatches =
        lastParagraphWithoutCodeBlocksAndUrlsAndMath.match(doubleUnderscorePattern)
    ...
    if (asteriskCount % 2 === 1) {
        ...
        if (
            isWithinMathBlock(content, absoluteLastStarPos, options) ||
        ...
        const afterLast = lastParagraphWithoutCodeBlocksAndUrlsAndMath
            .substring(lastParagraphWithoutCodeBlocksAndUrlsAndMath.lastIndexOf('**') + 2)
            .trim()
    ...
    if (underscoreCount % 2 === 1) {
        ...
        if (
            isWithinMathBlock(content, absoluteLastUnderscorePos, options) ||
        ...
        const afterLast = lastParagraphWithoutCodeBlocksAndUrlsAndMath
            .substring(lastParagraphWithoutCodeBlocksAndUrlsAndMath.lastIndexOf('__') + 2)
            .trim()
    ...
    if (endsWithSingleAsterisk && (needsAsteriskCompletion || needsAsteriskRemoval)) {
        ...
        const newLastParagraphWithoutCodeBlocksAndUrlsAndMath = removeMathBlocksFromText(
            newLastParagraphWithoutCodeBlocksAndUrls,
            options,
        )
        const newAsteriskMatches =
            newLastParagraphWithoutCodeBlocksAndUrlsAndMath.match(doubleAsteriskPattern)
        ...
        if (newAsteriskCount % 2 === 1) {
            ...
            const afterLast = newLastParagraphWithoutCodeBlocksAndUrlsAndMath
    ...
    if (endsWithSingleUnderscore && (needsUnderscoreCompletion || needsUnderscoreRemoval)) {
        ...
        const newUnderscoreMatches =
            newLastParagraphWithoutCodeBlocksAndUrlsAndMath.match(doubleUnderscorePattern)
        const newUnderscoreCount = newUnderscoreMatches ? newUnderscoreMatches.length : 0
        if (newUnderscoreCount % 2 === 1) {
            const lastUnderscorePos =
                newLastParagraphWithoutCodeBlocksAndUrlsAndMath.lastIndexOf('__')
            const afterLast = newLastParagraphWithoutCodeBlocksAndUrlsAndMath
    ...
    if (needsAsteriskCompletion) {
        ...
        if (!removedTrailingSingle) {
            ...
            const currentLastParagraphWithoutCodeBlocksAndUrlsAndMath = removeMathBlocksFromText(
                currentLastParagraphWithoutCodeBlocksAndUrls,
                options,
            )
            const withoutDoubleAsterisk =
                currentLastParagraphWithoutCodeBlocksAndUrlsAndMath.replace(
                    doubleAsteriskPattern,
                    '',
                )
