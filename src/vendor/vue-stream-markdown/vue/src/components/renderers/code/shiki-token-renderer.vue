...
export default defineComponent({
    ...
    setup(props) {
        ...
        return () => {
            if (!props.tokens?.tokens) return null

            return h(
                'pre',
                {
                    class: ['shiki', props.tokens.themeName, 'p-4 font-mono text-sm'],
                ...
                h(
                    ...
                    renderList(props.tokens.tokens, (line, index) =>
                        h(
                            'div',
                            {
                            ...
                            renderList(line, (token, tokenIndex) =>
                                h(
                                    'span',
                                    {
                                        key: tokenIndex,
                                        style:
                                            token.htmlStyle ||
                                            (getTokenStyleObjectRef.value?.(token) ?? {}),
                                    },
                                    token.content,
                                ),
                            ),
