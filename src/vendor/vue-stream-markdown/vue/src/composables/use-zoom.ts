import type { TouchZoomState, ZoomOptions, ZoomState } from '@stream-markdown/core'
import {
...
export function useZoom(options: ZoomOptions = {}) {
    ...
    function setState(state: Partial<ZoomState>) {
        ...
        if (typeof state.translateX === 'number') translateX.value = state.translateX

        if (typeof state.translateY === 'number') translateY.value = state.translateY
    }
