...
import { DEFAULT_LANGUAGE, loadLocaleConfig, resolveLocaleLanguage } from '@stream-markdown/core'
import { ref } from 'vue'
...
export async function loadLocaleMessages(language: string | LocaleConfig) {
    try {
        ...
        if (typeof language === 'string') currentLocale.value = resolveLocaleLanguage(language)
    } catch {
