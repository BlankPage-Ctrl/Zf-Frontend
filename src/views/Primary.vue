...
const wsFormSchema: DialogGridSchema = {
  ws: {
    columns: {
      name: { type: 'text-short', label: 'Name', placeholder: 'My workspace', metadata: { require: true } },
      description: { type: 'text-short', label: 'Description', placeholder: 'Optional description' },
      projectPath: { type: 'text-short', label: 'Project path', placeholder: '/path/to/project', metadata: { require: true } },
    },
  },
};
...
const chatFormSchema: DialogGridSchema = {
  chat: {
    columns: {
      ...
      model: { type: 'text-short', label: 'Model', placeholder: 'e.g. gpt-4o', metadata: { require: true } },
    },
  },
};
...
const wsListSchema = computed<ListSchema<Workspace>>(() => ({
  ...
  actions: [
    ...
    { icon: TrashIcon, ariaLabel: 'Delete', variant: 'danger', size: 'xs', onClick: confirmDeleteWs },
  ],
...
async function submitWsForm(data: DynamicGridDataOutput) {
  ...
  finally { wsFormLoading.value = false; }
}
...
async function submitChatForm(data: DynamicGridDataOutput) {
  ...
  finally { chatFormLoading.value = false; }
}
        ...
        <div class="ws-sidebar">
          <div class="ws-header">
            ...
            <div class="ws-header-actions">
              <pButton :schema="{ preset: 'icon-only', size: 'sm', icon: SettingsIcon, ariaLabel: 'Settings' }" @click="goToSettings" />
              <pButton :schema="{ preset: 'icon-only', size: 'sm', icon: PlusIcon, ariaLabel: 'New workspace' }" @click="openWsCreate" />
            </div>
          </div>
          ...
          <div v-else-if="wsStore.error && !wsStore.workspaces.length" class="ws-empty">
            ...
            <pButton :schema="{ preset: 'ghost', size: 'sm', label: 'Retry' }" @click="wsStore.fetchWorkspaces()" />
          </div>
      ...
      <template #cell-1-2>
        <div class="chat-content">
          ...
          <div v-if="!wsStore.selectedWorkspace" class="welcome-state">
            ...
            <p class="welcome-desc">Select a workspace from the sidebar to view and manage its chats, or create a new workspace to get started.</p>
          </div>
          ...
          <template v-else>
            <div class="chat-header">
              <div class="chat-header-info">
                <h2 class="chat-header-title">{{ wsStore.selectedWorkspace.name }}</h2>
                <span class="chat-header-meta">{{ chatStore.chats.length }} chat{{ chatStore.chats.length !== 1 ? 's' : '' }}</span>
              </div>
              <pButton :schema="{ preset: 'primary', size: 'sm', label: 'Open', icon: FolderIcon }" @click="openWorkspace" />
            </div>
